...
2. It has four tabs: Feed, Ads, Listings, Diagnostics. (The Legacy tab went with the WebView widget, which origin/main removed in 9ff579f. The Flutter sample went with the Flutter SDK, df551f7.)
...
1. One app: `bash scripts/e2e/run.sh ios`, `android`, `rn-ios` or `rn-android`.
...
2. `FEED_AD_ROWS`: the feed has ad rows that carry `sw.feed.ad` (set by the SDK feed). The feed flow then scrolls to one.
...
5. Time: about 2 minutes warm (123s on 2026-09-25 with four tabs, flow 55s). The first build also fetches the SwiftPM packages.
6. Not on iOS:
...
7. WebViews: Maestro can see into a WKWebView on iOS: the GAM test creative's iframe and images show in the hierarchy. The flows check only the containers.
...
9. Ids: the app's ids are Compose test tags, read as resource-ids (`testTagsAsResourceId`). The SDK feed's rows set their resource-id themselves (`SellwildFeedE2EIdTest`).
   1. A test tag on an `AndroidView` (such as `SellwildFeed`) does not reach UI Automator. The app puts `sw.feed.list` on a `Box` around the view.
10. Not on Android:
   1. House ad: `SellwildHouseAdView` is internal. House backfill runs inside `SellwildAdView` on a no-fill.
   2. Failure codes: there is no public failure sink. The screen shows the public `SellwildFailures.context` instead.
11. WebViews: debug builds turn on `WebView.setWebContentsDebuggingEnabled`. Maestro sees inside the ad creatives' WebViews on Android: the GAM creative's text shows in the hierarchy. The flows still check only the containers.
12. Time: about 2 minutes warm (103s on 2026-09-25 with four tabs, flow 46s). A cold build also downloads the Gradle dependencies.
...
6. Ids: `testID`. It is the accessibilityIdentifier on iOS and the resource-id on Android. A `View` around a native view (the feed, the ad slots) also sets `collapsable={false}`.
7. `rn-ios`:
   ...
   3. Time: about 4 minutes warm (255s on 2026-09-25 with four tabs, flow 45s; the Xcode build bundles the JS again every time). The first run took 549s, with `pod install` (hermes-engine, boost and the other pods download).
8. `rn-android`:

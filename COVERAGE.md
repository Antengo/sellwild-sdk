...
2. The React Native native bridge edits (react-native/ios, react-native/android) are not built in an RN host app here. `node react-native/native-checks/run.mjs` compiles them against React Native stand-ins (swiftc type-checks every react-native/ios file; the Android glue compiles with Kotlin 1.9 on the JVM) and runs their checks, but no gate step runs it. The RN package also needs a native SDK release that includes SellwildFailures.

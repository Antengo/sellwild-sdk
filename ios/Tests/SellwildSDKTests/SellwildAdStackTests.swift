...
final class SellwildAdStackTests: XCTestCase {
    ...
    func testResolveDefaultsToBoth() throws {
        XCTAssertEqual(SellwildAdStack.resolve(remoteValues: nil, zoneId: "43"), .both)
        XCTAssertEqual(
            SellwildAdStack.resolve(remoteValues: try AppConfigFactory.remote(["CODE": "weatherbug"]), zoneId: "43"),
            .both
        )
    }

    func testResolveGlobalHardWinsOverPerZone() throws {
    ...
    func testResolvePerZoneWhenNoGlobal() throws {
        let remote = try AppConfigFactory.remote([
    ...
    func testResolveOverrideBeatsRemote() throws {
        let remote = try AppConfigFactory.remote(["AD_STACK": "GAM"])
        XCTAssertEqual(

    ...
    private inner class RowAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
        ...
        override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
            ...
            when (val row = rows[position]) {
                ...
                is Row.GamAd -> (holder as AdHolder).view.bind(cfg, row.zoneId, row.gpid, ::onAdImpression, ::onHouseAdImpression, ::onAdClick, houseListingFor(position), ::handleFeedListingTap)
                is Row.DirectAd -> (holder as AdHolder).view.bind(cfg, row.zoneId, row.gpid, ::onAdImpression, ::onHouseAdImpression, ::onAdClick, houseListingFor(position), ::handleFeedListingTap)
                is Row.Banner -> (holder as AdHolder).view.bind(cfg, row.zoneId, row.gpid, ::onAdImpression, ::onHouseAdImpression, ::onAdClick, null, ::handleFeedListingTap)
            }
        }
    }

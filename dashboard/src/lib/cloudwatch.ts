...
function parseAuctionFromLog(logLine: string): ParsedAuction | null {
  try {
    ...
    // The logback pattern produces invalid JSON because %msg (which is JSON)
    // is embedded unescaped inside the outer JSON string. Extract the inner
    // JSON with a regex instead.
    const tsMatch = logLine.match(/"timestamp":"([^"]+)"/);
    const timestamp = tsMatch?.[1] || '';

    // Find the inner JSON: everything after "message":" up to the last ","containerId"
    ...
    // Determine source from debug.resolvedrequest or auction ID pattern
...
/** Get recent auctions from CloudWatch (last N hours, deduped by ID) */
export async function getRecentAuctions(hours = 24): Promise<ParsedAuction[]> {
  ...
  for (const row of rows) {
    ...
    if (!a && rows.indexOf(row) < 3) {
      console.log('[CW] Failed to parse:', msg.substring(0, 200));
    }
    if (a && !seen.has(a.id)) {
      seen.add(a.id);
      auctions.push(a);

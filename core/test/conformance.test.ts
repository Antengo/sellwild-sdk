...
// named in a case's knownDrift.core entry may differ, and must: the entry is
// removed in the change that fixes the drift.

interface ExpectationFile {
  zones: string[]
  fields: Record<string, { platforms: string[] }>
  cases: Array<{ file: string; expected: Record<string, unknown>; knownDrift: Record<string, string> }>
}
...
// Fields a knownDrift text names: "field:" anywhere, or the text starts with it.
...
function checkCase(doc: ExpectationFile, c: ExpectationFile['cases'][number], actual: Record<string, unknown>, views: Views = {}): void {
  const drift = driftFields(doc, c.knownDrift.core)
  for (const field of coreFields(doc)) {
    const expected = views[field] ? views[field](c.expected[field]) : c.expected[field]
    if (drift.includes(field)) {
...
describe('knownDrift handling', () => {
  // No case has a core entry today; this pins how one is treated when added.
  const doc: ExpectationFile = {
    zones: [],
    fields: { iabCats: { platforms: ['core'] }, slug: { platforms: ['core'] }, publisherId: { platforms: ['ios'] } },
    cases: [],
  }
  const drifting = { file: 'x.json', expected: { iabCats: ['IAB15'], slug: 's', publisherId: '1' }, knownDrift: { core: 'iabCats: core reads only a list' } }

  it('lets a named field differ, and still checks the others', () => {
    expect(() => checkCase(doc, drifting, { iabCats: [], slug: 's' })).not.toThrow()
    expect(() => checkCase(doc, drifting, { iabCats: [], slug: 'other' })).toThrow('x.json slug')
  })

  it('fails once the drift is fixed, so the entry gets removed', () => {
    expect(() => checkCase(doc, drifting, { iabCats: ['IAB15'], slug: 's' })).toThrow('listed as known drift')
  })

  it('has no core drift entries today', () => {
    for (const file of ['expectations/app-config.expected.json', 'expectations/listings-response.expected.json']) {
      const cases = contract<ExpectationFile>(file).cases.filter((c) => c.knownDrift.core !== undefined)
      expect(cases.map((c) => c.file), file).toEqual([])
    }
  })
})
...
describe('app-config conformance (core)', () => {
  ...
  it.each(appDoc.cases.map((c) => [c.file, c] as const))('%s', async (_file, c) => {
    ...
    checkCase(appDoc, c, appConfigResult(config, raw), { mobileZids: (v) => (v as { shared: unknown }).shared })
  })
})

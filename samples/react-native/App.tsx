...
const BIDDERS = {
  appnexus: { placement_id: 13144370 },
  pubmatic: { publisherId: '12345', adSlot: 'demo' },
  ix: { siteId: '123456' },
...
async function runMultiSlotAuction(): Promise<MultiAuctionResult> {
  ...
  try {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (allBids.length > 0 || attempt === 2) {
        return { loading: false, slots, totalTimeMs, responseTimes }
      }
    }
  } catch (e: any) {
    return {
  ...
  return { loading: false, slots: [], totalTimeMs: Date.now() - start, responseTimes: {} }
}
...
function AuctionPanel({ slots, onRefresh }: { slots: SlotAuction[]; onRefresh: () => void }) {
...
function NativeScreen() {
  ...
  const [auctionResult, setAuctionResult] = useState<MultiAuctionResult>({ loading: true, slots: [], totalTimeMs: 0, responseTimes: {} })
  ...
  return (
    <ScrollView style={s.flex} contentContainerStyle={s.scrollContent} showsVerticalScrollIndicator={false}>
      ...
      {!auctionsLoading && bannerAuction && mrecAuction && (
        <AuctionPanel slots={[bannerAuction, mrecAuction]} onRefresh={runAuctions} />
      )}
      {auctionsLoading && (

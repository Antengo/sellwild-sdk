...
Shared, language-neutral contracts for every Sellwild client: TS core, React Native, iOS, Android and the web widget. (The Flutter SDK was removed on origin/main, df551f7.) Nothing here ships to partners.

```
npm install          # ajv 8 + ajv-formats, dev only
npm test             # node --test: vectors, validation, registry, mirrors, add-code, drift, key coverage, print gate, refresh guard
npm run add-code -- '<json entry>'   # the only way to change failure-codes.json (FAILURES.md 4.4)
...
| `schemas/` | JSON Schema 2020-12 for every external shape: `app-config`, `listing`, `listings-response`, `localized-listings-config`, `localized-listings-response`, `events-batch`, `client-failure-event`, `rn-native-config`, `growthcode-sync-response`, `eid-blob`, plus `failure-codes`. |
...
3. **Conformance.** Each platform runs `fixtures/app-config/valid/*` and the app-config samples through its real parser and compares with `expectations/app-config.expected.json` (same for listings). A difference is allowed only in a field named in that platform's entry for the case in `expectations/drift/<platform>.json` (`expectations.<file>.<case>`). Remove the entry in the change that fixes the drift. FAILURES.md section 14.
4. **Golden vectors.** Each pure core replays `golden/log-failure.vectors.json` (TS and Kotlin also the `utf16` file) and must match every vector exactly. See FAILURES.md section 12.
5. **Registry parity.** Each platform mirrors `failure-codes.json` as constants, generated by `scripts/gen-codes.mjs` (never edited by hand), and tests that the mirror equals the codes whose `clients` include it.
...
3. New failure code, or a client added to or dropped from one: `node scripts/add-code.mjs ...` (FAILURES.md section 4.4). It is safe to run from several units at once.

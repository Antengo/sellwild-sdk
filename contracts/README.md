...
| `failure-codes.json` | Canonical failure-code registry. `failure-codes.sources.json` traces each code to the phase-1 failure points (or its `added` record). Change it only with `scripts/add-code.mjs`, which also regenerates the four platform mirrors (`scripts/gen-codes.mjs`). |

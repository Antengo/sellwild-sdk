...
2. Never rename or remove an id. Flows on other platforms use it. (The Legacy tab's ids went with the WebView widget, which origin/main removed in 9ff579f.)
...
4. The app sets ids on its own containers:
   ...
   3. React Native: `testID`. On a plain `View` around a native view, also set `collapsable={false}`, so Android keeps the view.
...
4. The React Native sample's jest tests (`samples/demo-app/__tests__/App.test.tsx`) find its tab and Diagnostics ids by `testID`.
